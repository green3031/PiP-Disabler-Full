...
namespace PiPDisabler
{
    public static class Settings
    {
        ...
        // --- 0. Display language ---
        // Stable English cfg key. Its value is "Auto" (= follow the game) or a language code
        // such as "English" / "中文" / "zh-CN" / "en". The visible text of every entry (and of
        // every section header) comes from <plugin dir>\i18n\<code>.json - see I18n.cs and
        // i18n\README.md. The cfg section and key names below NEVER change with the language.
        ...
        // --- Mesh Surgery: auto-derived cut region (from the optic's own geometry) ---
        public static ConfigEntry<bool> AutoDeriveCutRegion;
        public static ConfigEntry<float> AutoDeriveMargin;
        public static ConfigEntry<float> AutoDeriveExtraRadiusMeters;
        public static ConfigEntry<float> AutoDeriveMaxRadiusMeters;
        public static ConfigEntry<bool> AutoDeriveLogProfile;
        ...
        {
            // --- Display language (bound first so it sorts to the top of the General section) ---
            // The text below is only the built-in fallback: the i18n language file overrides it.
            ConfigEntries.Add(Language = config.Bind("General", "Language", I18n.AutoValue,
                new ConfigDescription(
                    "Display language of this mod's settings.\n" +
                    "Auto = follow the game's in-game language setting (recommended).\n" +
                    "You can also type a language code directly, e.g. en, zh-CN, ru.\n" +
                    "English = en, 中文 = zh-CN.\n" +
            ...
            ConfigEntries.Add(AutoDeriveMargin = config.Bind("Global Mesh Surgery settings", "Auto derive margin", 1.15f,
                new ConfigDescription(
                    "Multiplier applied to the derived radius: R = min(max(R_geo, R_manual) * margin + extra, max radius).",
                    new AcceptableValueRange<float>(0.5f, 3f),
                    new ConfigurationManagerAttributes { IsAdvanced = false })));
            ConfigEntries.Add(AutoDeriveExtraRadiusMeters = config.Bind("Global Mesh Surgery settings", "Auto derive extra radius meters", 0.005f,
                new ConfigDescription(
                    "Constant added (meters) after the margin multiplier.",
                    new AcceptableValueRange<float>(0f, 0.05f),
                    new ConfigurationManagerAttributes { IsAdvanced = true })));
            ConfigEntries.Add(AutoDeriveMaxRadiusMeters = config.Bind("Global Mesh Surgery settings", "Auto derive max radius meters", 0.25f,
                new ConfigDescription(
                    "Upper clamp (meters) for every derived radius. The hand-tuned value is still\n" +
                    "enforced as a floor, so this clamp can never make the cut smaller than before.",
                    new AcceptableValueRange<float>(0.01f, 0.5f),
                    new ConfigurationManagerAttributes { IsAdvanced = false })));
            ConfigEntries.Add(AutoDeriveLogProfile = config.Bind("Global Mesh Surgery settings", "Auto derive log derived profile", false,
                new ConfigDescription(
                    "Log the derived cut profile once per derivation (one line, no per-frame cost).\n" +
                    "Independent of 'Debug logging' so it can be enabled on its own.",
                    null,
                    new ConfigurationManagerAttributes { IsAdvanced = true })));
            ...
            // Display layer only: pushes the translated DispName/Description/Category into the
            // ConfigurationManagerAttributes of every entry above. Runs after RecalcOrder so the
            // two tag mutations cannot interfere, and before ConfigurationManager ever reads the
            // tags (its setting list is built when the F12 window opens).
