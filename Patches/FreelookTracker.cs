...
namespace PiPDisabler
{
    ...
    /// CameraManager.SetFov(35, 1, true) for optics.  This hardcoded 35°
    ...
    /// restore it:
    ///   1. OnFreelookExit() calls CameraManager.SetFov directly (Update path).
    ...
    /// case OnFreelookEnter fires before CameraManager is valid.
    /// </summary>
    internal static class FreelookTracker
    {
        ...
        /// <summary>
        /// Drop-in replacement for CameraManager.SetFov called from Player.Look.
        ...
        ///
        /// The signature matches CameraManager.SetFov(float, float, bool) exactly
        /// so the transpiler can do a simple callvirt→call swap.
        /// </summary>
        public static void LookSetFovInterceptor(CameraManager cameraManager, float targetFov, float duration, bool force)
        {
    ...
    {
        /// <summary>
        /// Transpiles Player.Look to replace CameraManager.SetFov with
