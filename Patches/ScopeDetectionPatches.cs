...
namespace PiPDisabler.Patches
{
    internal sealed class OpticSightOnEnablePatch : ModulePatch
    {
        protected override MethodBase GetTargetMethod()
            => AccessTools.Method(typeof(OpticSight), "OnEnable", Type.EmptyTypes);
    ...
    {
        protected override MethodBase GetTargetMethod()
            => AccessTools.Method(typeof(OpticSight), "OnDisable", Type.EmptyTypes);
    ...
    internal sealed class TacticalRangeFinderIgnoreLocalBodyPatch : ModulePatch
    {
        ...
        protected override MethodBase GetTargetMethod()
            => AccessTools.Method(typeof(TacticalRangeFinderController), "MeasureDistance");
    ...
    /// <summary>
    /// Postfix on Player.OnSetInHands(SetInHandsEventArgs).
    ...
    ///
    /// 4.1.5 note: EFT.Player exposes this only as an explicit interface implementation,
    /// so its metadata name is "EFT.ISetInHandsHandler.OnSetInHands" and
    /// AccessTools.Method(typeof(Player), "OnSetInHands") returns null. Two lookups are
    /// tried; if both fail the patch is skipped safely by Patcher (IsTargetAvailable()).
    /// </summary>
    internal sealed class PlayerOnSetInHandsPatch : ModulePatch
    {
        private static readonly Type[] SetInHandsParamTypes = new[] { typeof(SetInHandsEventArgs) };

        protected override MethodBase GetTargetMethod()
            => FindTargetMethod();

        /// <summary>
        /// Used by Patcher to pre-check: when false the patch is not registered at all.
        /// </summary>
        internal static bool IsTargetAvailable()
            => FindTargetMethod() != null;

        internal static MethodBase FindTargetMethod()
        {
            const BindingFlags flags = BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic;

            // Lookup A: explicit interface implementation metadata name.
            try
            {
                var explicitImpl = AccessTools.Method(
                    typeof(Player), "EFT.ISetInHandsHandler.OnSetInHands", SetInHandsParamTypes);
                if (explicitImpl != null) return explicitImpl;
            }
            catch { /* fall through to lookup B */ }

            // Lookup B: name suffix + single parameter type (survives future renames).
            try
            {
                var methods = typeof(Player).GetMethods(flags);
                for (int i = 0; i < methods.Length; i++)
                {
                    var m = methods[i];
                    if (m == null || !m.Name.EndsWith("OnSetInHands", StringComparison.Ordinal)) continue;
                    var ps = m.GetParameters();
                    if (ps.Length == 1 && ps[0].ParameterType == typeof(SetInHandsEventArgs))
                        return m;
                }
            }
        ...
        [PatchPostfix]
        private static void Postfix(Player __instance, SetInHandsEventArgs eventArgs)
        {
            if (!Settings.ModEnabled.Value) return;
