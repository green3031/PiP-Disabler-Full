...
namespace PiPDisabler
{
    public static class MeshSurgeryManager
    {
        ...
        private static bool _autoDeriveFailureLogged;
        ...
        private static void RebuildCutCacheForOptic(CutProfileCache cache, OpticSight os, Transform scopeRoot, Transform activeMode)
        {
            ...
            // The four-plane cut profile is loop invariant, so it is built ONCE per cache rebuild
            // (i.e. on scope change / settings change) and never per frame. In auto mode this is
            // also the single place where the optic geometry is measured.
            var cutProfile = BuildCutProfile(os, scopeRoot, planePoint, planeNormal, targets);
            ...
            foreach (var mf in targets)
            {
                ...
                try
                {
                    ...
                    if (isCylinder)
                    {
                        // cutProfile is either the untouched hand-tuned per-scope profile
                        // (auto mode off) or the geometry-derived one (auto mode on).
                        ok = MeshPlaneCutter.CutMeshFrustum(readable, mf.transform,
                            planePoint, planeNormal, cutProfile.NearRadius, cutProfile.Plane4Radius,
                            cutProfile.StartOffset, cutProfile.CutLength,
                            keepInside: false, midRadius: cutProfile.MidRadius, midPosition: cutProfile.MidPosition,
                            nearPreserveDepth: cutProfile.NearPreserveDepth,
                            plane3Radius: cutProfile.Plane3Radius, plane3Position: cutProfile.Plane3Position,
                            plane4Position: cutProfile.Plane4Position);
                    }
                    else
                    {
        ...
        /// <summary>
        /// Same active-mode resolution the cut itself uses (an optic whose own transform is a
        /// "mode_*" node IS the mode node).
        /// </summary>
        ...
        /// <summary>
        /// Builds the cut profile handed to <see cref="MeshPlaneCutter.CutMeshFrustum"/>.
        ///
        /// Frustum mode OFF + auto mode OFF (the default, and exactly the pre-existing path): the
        /// hand-tuned per-scope values are read through exactly the same getters and in the same
        /// form as before, so behaviour is unchanged. When frustum mode is off this method cannot
        /// take a different branch at all - the new code below is skipped as a whole.
        /// Frustum mode ON: the profile the player computed with the frustum key is used for the
        /// four radii/positions; the window parameters (offset / length / preserve depth) still
        /// come from the per-scope values. With no stored profile yet, the previous path runs
        /// unchanged and one line explains why.
        /// Auto mode ON (legacy): <see cref="AutoCutProfileDeriver"/> widens the radius envelope
        /// with the optic's measured geometry; the composition is a max(), so the result is never
        /// weaker than the hand-tuned profile.
        ///
        /// Any failure inside a derivation logs ONE warning and falls back - it must never throw
        /// into the game loop and must never stop the cut.
        /// </summary>
        private static CutProfileParams BuildCutProfile(OpticSight os, Transform scopeRoot,
            Vector3 planePoint, Vector3 planeNormal, List<MeshFilter> targets)
        {
            float cutLength = PerScopeMeshSurgerySettings.GetCutLength();

            var manual = new CutProfileParams
            {
                NearRadius = PerScopeMeshSurgerySettings.GetPlane1Radius(),
                MidRadius = PerScopeMeshSurgerySettings.GetPlane2Radius(),
                MidPosition = PerScopeMeshSurgerySettings.GetPlane2PositionNormalized(cutLength),
                Plane3Radius = PerScopeMeshSurgerySettings.GetPlane3Radius(),
                Plane3Position = PerScopeMeshSurgerySettings.GetPlane3Position(),
                Plane4Radius = PerScopeMeshSurgerySettings.GetPlane4Radius(),
                Plane4Position = PerScopeMeshSurgerySettings.GetPlane4Position(),
                StartOffset = PerScopeMeshSurgerySettings.GetCutStartOffset(),
                CutLength = cutLength,
                NearPreserveDepth = PerScopeMeshSurgerySettings.GetNearPreserveDepth(),
            };

            if (!Settings.AutoDeriveCutRegion.Value)
                return manual;

            // The hand-tuned numbers come from the per-scope JSON override ONLY when a row matches
            // the active scope key; otherwise they come from the global config entries. Label the
            // log line accordingly - naming a JSON key whose values were not used would send a
            // tuner to the wrong row.
            string overrideKey = PerScopeMeshSurgerySettings.GetActiveScopeKey();
            string scopeLabel = string.IsNullOrEmpty(overrideKey) ? "<global>" : overrideKey;
            string opticLabel = os != null && !string.IsNullOrEmpty(os.name) ? os.name : "<none>";

            try
            {
                return AutoCutProfileDeriver.Derive(os, scopeRoot, planePoint, planeNormal,
                    targets, manual, scopeLabel, opticLabel);
            }
            catch (Exception ex)
            {
                LogAutoDeriveFailure(ex);
                return manual;
            }
        }

        /// <summary>
        /// One unconditional warning for the first derivation failure of the PROCESS (same
        /// "fail once, then stay quiet" pattern as the JSON backend guard). Deliberately NOT
        /// behind Settings.DebugLogging: silently using the hand-tuned profile is something the
        /// player has to be able to see with the default config.
        /// </summary>
        private static void LogAutoDeriveFailure(Exception ex)
        {
            if (_autoDeriveFailureLogged)
                return;

            _autoDeriveFailureLogged = true;

            string message =
                $"[MeshSurgery][Auto] Geometry derivation failed - {ex.GetType().Name}: {ex.Message}. " +
                "Falling back to the hand-tuned per-scope cut profile for this rebuild.";
        ...
        private static string BuildCutSettingsSignature()
        {
            return string.Join("|", new[]
            {
                ...
                // Auto-derivation inputs: they change the cut geometry, so toggling any of them
                // must rebuild the cached cut meshes. The log switch is deliberately NOT part of
                // the signature - it does not change geometry and must not force a rebuild.
                Settings.AutoDeriveCutRegion.Value ? "Auto=1" : "Auto=0",
                Settings.AutoDeriveMargin.Value.ToString("F4"),
                Settings.AutoDeriveExtraRadiusMeters.Value.ToString("F4"),
                Settings.AutoDeriveMaxRadiusMeters.Value.ToString("F4"),
            });
        }
